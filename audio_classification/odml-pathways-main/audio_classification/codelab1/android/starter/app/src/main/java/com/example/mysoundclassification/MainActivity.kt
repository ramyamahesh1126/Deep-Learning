...
//             val output = classifier.classify(tensor)
            ...
            var filteredModelOuput = output[0].categories.filter {
...
//             }
            ...
            if (filteredModelOuput.isNotEmpty()) {
                Log.i("Yamnet", "bird sound detected!")
                filteredModelOuput = output[1].categories.filter {
                    it.score > probabilityThreshold
                }
            }
            // TODO 4.3: Creating a multiline string with the filtered results
            val outputStr = filteredModelOuput.sortedBy { -it.score }
